...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template name="encode-value">
		...
		<xsl:choose>
			<xsl:when test="(string(number($value)) = 'NaN' or (substring($value , string-length($value), 1) = '.') or (substring($value, 1, 1) = '0') and not($value = '0')) and not($value = 'false') and not($value = 'true') and not($value = 'null')">
				<xsl:text>"</xsl:text>
				<xsl:call-template name="encode">
